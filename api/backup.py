"""
Backup and rollback for critical files.
3 versions: lts (manually marked), previous, current.
Automatic rollback if service fails after modification.
"""
import hashlib
import asyncio
import asyncssh
...
async def backup_file(pool, module_name: str, created_by: str = "argos"):
    ...
    try:
        ...
        async with pool.acquire() as conn:
            ...
            # Delete old previous (keep only one)
            await conn.execute(
