...
async def compress_conversation(req: CompressRequest):
    ...
    # Old messages to compress (all except last N)
    ...
    try:
        response = client.messages.create(
            ...
            messages=[{
                "role": "user",
                "content": f"""Make a concise technical summary of this conversation.
Keep: decisions made, configurations discussed, problems solved, important files, IPs, key commands, current project state.
Ignore: greetings, rhetorical questions, simple confirmations, small talk.
Format: short paragraph per major topic, no excessive bullet points.
...
{text_to_summarize}"""
    ...
    # Save segment to DB and delete old messages
    async with pool.acquire() as conn:
