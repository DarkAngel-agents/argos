...
KNOWN_HOSTS = {
    "database": {"host": "11.11.11.111", "user": "darkangel"},
...
LOCAL_MACHINES = ["beasty", "11.11.11.111"]
...
async def nixos_rebuild(req: NixosRebuildRequest):
    ...
    backup = await _exec_ssh_by_name("database", backup_cmd)
...
async def list_backups():
    result = await _exec_ssh_by_name("database", f"ls -lt {BACKUP_DIR}/*.nix 2>/dev/null | awk '{{print $6, $7, $9}}'")
...
async def restore_backup(req: RestoreRequest):
    ...
    if read["returncode"] == 0:
        ...
        await _exec_ssh_by_name("database", f"printf '%s' '{escaped}' > {BACKUP_DIR}/{pre_restore_backup}")

    read_backup = await _exec_ssh_by_name("database", f"cat {BACKUP_DIR}/{req.filename}")
