...
async def auto_archive(pool: asyncpg.Pool):
    ...
    for conv in conversations:
        ...
        async with pool.acquire() as conn:
            ...
            # Delete raw messages (keep first and last)
            await conn.execute(
...
async def vacuum_old_data(pool: asyncpg.Pool):
    """Delete old data from client monitoring schema"""
    ...
    async with pool.acquire() as conn:
        ...
        if dh:
            ...
            print(f"[VACUUM] Deleted {dh} old rows from device_history")
        ...
        if ev:
            ...
            print(f"[VACUUM] Deleted {ev} old events")
        ...
        if logs:
            ...
            print(f"[VACUUM] Deleted {logs} old log entries")
